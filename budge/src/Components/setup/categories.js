      ...
      {progress === 0 ? (
        ...
        <div className="categories progress-1">
          ...
          {categories.map((category) => {
            return (
              <>
                {category.category_id >= 8 && category.category_id <= 15 ? (
                  <button key={category.category_id}>
                    {category.category_name}
                  </button>
                ) : null}
        ...
        <div className="categories progress-2">
          <h1>Insurance</h1>
          <h2>
            You are stunning! All that pizzaz must require some maintenance
          </h2>
          <h3>Who is giving you that trusted care?</h3>
          {categories.map((category) => {
            return (
              <>
                {category.category_id >= 16 && category.category_id <= 20 ? (
                  <button key={category.category_id}>
                    {category.category_name}
                  </button>
                ) : null}
        ...
        <div className="categories progress-3">
          ...
          {categories.map((category) => {
            return (
              <>
                {category.category_id >= 21 && category.category_id <= 25 ? (
                  <button key={category.category_id}>
                    {category.category_name}
                  </button>
                ) : null}
        ...
        <div className="categories progress-4">
          ...
          {categories.map((category) => {
            return (
              <>
                {category.category_id >= 26 && category.category_id <= 27 ? (
                  <button key={category.category_id}>
                    {category.category_name}
                  </button>
                ) : null}
        ...
        <div className="categories progress-5">
          ...
          {categories.map((category) => {
            return (
              <>
                {category.category_id >= 28 && category.category_id <= 30 ? (
                  <button key={category.category_id}>
                    {category.category_name}
                  </button>
                ) : null}
